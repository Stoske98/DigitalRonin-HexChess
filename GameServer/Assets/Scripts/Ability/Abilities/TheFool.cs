...
public class TheFool : PassiveAbility
{
    ...
    {
        foreach (var hex_neighbor in _hex.GetNeighbors(NetworkManager.Instance.games[unit.match_id].map))
        {
...
public class TheFoolFinal : InstantleAbility, ISubscribe
{
    ...
    private void OnRecieveDamage(Hex _hex)
    {
        foreach (Hex neighbor_hex in NetworkManager.Instance.games[unit.match_id].map.HexesInRange(_hex, ability_data.range))
        {
