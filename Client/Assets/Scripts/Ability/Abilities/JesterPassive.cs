...
public class JesterPassive : PassiveAbility
{
    ...
    private void OnStartMovement(Hex from_hex, Hex _target_hex)
    {
        ...
        if (behaviour != null && behaviour is NormalMovement)
        {
            if (from_hex != null)
            {
                ...
                {
                    int rand = (int)MathF.Floor(GameManager.Instance.game.random_seeds_generator.GetRandomSeed() * walkable_hexes.Count);
                    ...
                    if (walkable_hexes.Count != 0)
                    {
                        rand = rand = (int)MathF.Floor(GameManager.Instance.game.random_seeds_generator.GetRandomSeed() * walkable_hexes.Count);
    ...
    {
        GameManager.Instance.game.GetHex(_illusion)?.RemoveUnit();
        GameManager.Instance.game.PlaceObject(_illusion, _parent_hex);
