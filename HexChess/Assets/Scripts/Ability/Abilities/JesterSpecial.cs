...
public class JesterSpecial : PassiveAbility
{
    ...
    {
        GameManager.Instance.game.map.GetHex(_illusion)?.RemoveObject(_illusion);
...
public class JesterSpecialFinal : TargetableAbility, ITargetMultipleHexes, ISubscribe
{
    ...
    public override List<Hex> GetAbilityMoves(Hex _unit_hex)
    {
        List<Hex> _available_moves = PathFinding.PathFinder.BFS_HexesMoveRange(_unit_hex, ability_data.range, GameManager.Instance.game.map);
